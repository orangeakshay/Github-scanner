...
const getRepositoryDetails = (options = {})=> {
  ...
  `;
      ...
      result.push({
        ...
        "RepositorySize": diskUsage,
        "RepositoryOwner": login,
        "RepositoryPrivate": isPrivate,
        "NumberOfFileOnRootLevel": fileCount,
        "NumberOfFoldersOnRootLevel": folderCount,
        "HaveYMLFile": haveYmlFile,
        "ContentOfYMLFile": text,
...
const checkYml = (options = {}) =>{
  ...
  return new Promise(async(resolve, reject)=>{
    try{
      ...
      return resolve({text:text,haveYmlFile:false});
    }catch(err){
      console.error(methodName, err)
      return reject(err)
    }
